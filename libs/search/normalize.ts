...
function projectReferences(
  sources: Array<{
    references?:
      | ChapterNote["references"]
      | NovelEvent["description_references"];
  }>,
  entityMap: EntityMap,
) {
  ...
  for (const source of sources) {
    for (const occurrence of source.references ?? []) {
      if (occurrence.token.status === "resolved")
        ids.add(occurrence.token.reference.entityId);
    }
  }
  const entities = [...ids]
...
export const volumeRoute = (novelId: string, volumeId: string) =>
  `${novelRoute(novelId)}/volumes/${volumeId}`;
export const chapterRoute = (
  novelId: string,
  volumeId: string,
  chapterId: string,
) => `${volumeRoute(novelId, volumeId)}/chapters/${chapterId}`;
export const adaptationRoute = (novelId: string, adaptationId: string) =>
  `${novelRoute(novelId)}/adaptations#adaptation-${adaptationId}`;
...
export function normalizeVolume(
  ...
  return {
    ...
    ...emptyFields(),
    aliases: [...new Set([volume.title_en, volume.title_th].filter(Boolean))],
...
export function normalizeChapter(
  ...
  return {
    ...
    name: formatChapterLabel(chapter, labels),
    title: chapter.title,
    description: chapter.description,
    ...projectReferences(chapter.notes, entityMap),
    aliases: [...new Set(titles)],
    tagIds: chapter.tags.map((tag) => tag.id),
...
export function normalizeNote(
  ...
  return {
    ...
    ...projectReferences([note], entityMap),
    tagIds: tags.map((tag) => tag.id),
    tags: tags.map((tag) => tag.name),
    route: `${chapterRoute(novelId, volumeId, chapterId)}?note=${encodeURIComponent(note.id)}`,
  };
}
...
export function normalizeEntity(entity: Entity): SearchDocument {
  ...
  const route =
    entity.type === "character"
      ? `${novelRoute(entity.novelId)}/characters/${sourceId}`
      : `${novelRoute(entity.novelId)}/entities/${encodeURIComponent(entity.id)}`;
  return {
    id: `entity:${entity.id}`,
    type: "entity",
...
export function normalizeEvent(
  ...
  const linkedLabel = linkedChapter
    ? formatChapterLabel(linkedChapter, labels)
    : (event.chapter_title ?? "");
  return {
    ...
    eventId: event.id,
    title: event.title,
    content: `${event.description} ${linkedLabel}`.trim(),
    ...projectReferences(
...
export function normalizeAdaptation(
  ...
  return {
    id: `adaptation:${adaptation.novel_id}:${adaptation.volume_id}:${adaptation.id}`,
    type: "adaptation",
    novelId: adaptation.novel_id,
    volumeId: adaptation.volume_id,
    adaptationId: adaptation.id,
    title: adaptation.title,
    description: adaptation.description,
    content: [
      adaptation.medium,
      adaptation.group_label,
