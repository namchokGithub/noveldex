...
- [x] Post-migration cleanup — removed Go/Redis/HTTP-search runtime, moved the runtime app to the repository root, consolidated active documentation, and retained PostgreSQL backup recovery commands
...
## Phase 3: Timeline (partial — remaining item still in PROGRESS.md)
...
## Phase 3.1: Chapter Notes (ships ahead of full-text search)
...
## Phase 4.6: My Polish (partial — remaining items still in PROGRESS.md)
...
- [x] Firestore adaptation documents at `novels/{novelId}/volumes/{volumeId}/adaptations/{adaptationId}` with parent-context fields, enum/URL validation, per-volume ordering, explicit volume-delete cascade, and a `novel_id` collection-group index config
...
- [x] Cross-browser / device verification — manual verification completed; no issues reported in this round.
...
- [x] Adaptation detail — dedicated read-only route with parent Volume, mapped Chapters, and generic Story entities from persisted adaptation-note references; Timeline titles link to it and Notes returns to it
