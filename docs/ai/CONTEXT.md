...
`novels/{novelId}/entityReferences` is a derived inverse index for resolved entity references in Chapter notes, Event descriptions, and Adaptation notes. Source documents remain authoritative; their create, update, delete, and reorder mutations maintain deterministic index entries. Entity detail initially reads only its Entity document, then loads cross-references on demand with a cursor-bounded query. Adaptations related through indexed Chapter IDs are fetched in `array-contains-any` chunks of at most 30 IDs. When reconciling legacy data, first run `corepack pnpm backfill:entity-references -- --project <id> --dry-run` and apply it, then dry-run, apply, and verify `corepack pnpm backfill:entity-reference-index -- --project <id>` in an approved maintenance window. Deploy the `entityReferences` index from `firestore.indexes.json` before enabling the reader.

Character documents additionally keep optional rich profile content (`appearance`, `personality`, and `trivia`, with their Tiptap JSON documents), a `data` fact sheet grouped as biographical/biological, social, and debut data, and an ordered `gallery[]` of external image metadata. Gallery image URLs are HTTP(S) only; images are displayed only on the Character detail page and are not hydrated into other views. The directory maintains a lowercased, trimmed `name_search` field for prefix search; create/update writes keep it current. Before enabling prefix search for legacy data, deploy the Character indexes in `firestore.indexes.json`, then run `corepack pnpm backfill:character-name-search -- --project <id>` as a dry run and repeat with `--apply`.

The Character directory supports role filtering, prefix name search, and name, updated-at, or role sorting in either direction. Its normal non-search path is cursor-bounded; prefix search intentionally reads all prefix matches so its results can be sorted and paginated consistently in the client.
...
- Phase 3 supplies `libs/search/SearchIndexProvider.tsx`: it mounts once inside `I18nProvider`, builds one session-wide MiniSearch index incrementally from the current novel first, and expands to remaining novels only when global scope is requested. It keeps separate document/entity/dependency maps because MiniSearch uses `storeFields: []`, deduplicates loaded and in-flight novel datasets, and supports serialized incremental add/replace/discard batches, reference-projection refresh, and vacuum maintenance; Thai/English tokenization uses `Intl.Segmenter` with a fallback. `SearchScope`, staged ranking, and debounce live in the command palette. The synthetic benchmark report records the growth checkpoints; do not add IndexedDB or a Worker without renewed benchmark evidence.
