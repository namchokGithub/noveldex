...
Adaptations are owned by a volume at `novels/{novelId}/volumes/{volumeId}/adaptations/{adaptationId}`. Each document denormalizes `novel_id` and `volume_id` for the novel-wide collection-group query; keep those values aligned with its parent path. Adaptation `sort_order` is scoped to its volume and medium/group, while the timeline groups entries by volume then medium/group. The collection-group query requires the `adaptations` `novel_id` field override in `firestore.indexes.json` to be deployed before use.
