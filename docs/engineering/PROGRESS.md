...
- [x] Plan 4 — Postgres→Firestore production migration completed from `backups/postgres/noveldex-20260828-233248.sql` using a disposable PostgreSQL 16 restore. Dry-run found no integrity errors; migrated source counts were 1 novel, 5 roles, 23 volumes, 4 chapters, 1 character, 2 events, and 1 tag. Post-write verification confirmed 1 `novels` and 5 `character_roles` documents. The CLI's current `--verify-only` checks those top-level collections only (`docs/superpowers/plans/2026-08-28-firebase-data-migration-and-cutover.md`).
- [ ] Post-migration production validation — deploy/validate `firestore.indexes.json` against the real Firebase project, then run a browser smoke test for every migrated domain. Include nested Firestore collections and `chapterNumbers` markers, which the current `--verify-only` command does not count.
