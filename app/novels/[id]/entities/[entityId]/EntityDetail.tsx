...
import { ConfirmDialog, inputClassName, primaryButtonClassName, secondaryButtonClassName } from "../../../ui";
...
export default function EntityDetail({ novelId, entity }: { novelId: string; entity: Entity }) {
  const { t } = useI18n(); const { upsert, discard } = useSearchMutations(); const router = useRouter(); const [name, setName] = useState(entity.name); const [aliases, setAliases] = useState(entity.aliases.join(", ")); const [description, setDescription] = useState(entity.description); const [busy, setBusy] = useState(false); const [confirming, setConfirming] = useState(false);
  async function save() { setBusy(true); try { const updated = await updateEntity(novelId, entity.id, { name: name.trim(), aliases: aliases.split(",").map((value) => value.trim()).filter(Boolean), description }); upsert(normalizeEntity(updated)); router.refresh(); } finally { setBusy(false); } }
  async function remove() { setBusy(true); try { await deleteEntity(novelId, entity.id); discard(`entity:${entity.id}`); router.push(`/novels/${novelId}/entities`); } finally { setBusy(false); } }
