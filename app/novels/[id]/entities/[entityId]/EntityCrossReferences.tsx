...
export default function EntityCrossReferences({
  ...
  const [nextCursor, setNextCursor] = useState<EntityReferenceCursor | null>(
    null,
  );
    ...
    try {
      ...
      setLinkedAdaptations((current) =>
        cursor
          ? [
              ...new Map(
                [...current, ...linked].map((item) => [item.id, item]),
              ).values(),
            ]
          : linked,
      );
  ...
  const notes = (items ?? []).filter(
    (item) => item.source_type === "chapter_note",
  );
  const events = (items ?? []).filter((item) => item.source_type === "event");
  const adaptations = [
    ...new Map([
      ...(items ?? [])
        .filter((item) => item.source_type === "adaptation_note")
        .map(
          (item) =>
            [
              item.source_id,
              { id: item.source_id, title: item.title },
            ] as const,
        ),
      ...linkedAdaptations.map(
        (item) => [item.id, { id: item.id, title: item.title }] as const,
      ),
    ]).values(),
  ];
    ...
    <section className={cardClassName}>
      ...
      {items === null ? (
        <button
          ...
          disabled={loading}>
          {loading
            ? t("entities.loadingRelatedRecords")
            : t("entities.loadRelatedRecords")}
        </button>
      ) : (
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
          {notes.length > 0 ? (
            <RelatedGroup title={t("entities.notes")}>
              {notes.map((item) => (
                <Link
                  key={item.id}
                  href={`/novels/${novelId}/volumes/${item.volume_id}/chapters/${item.source_id}?note=${encodeURIComponent(item.note_id ?? "")}`}
          ...
          {events.length > 0 ? (
            <RelatedGroup title={t("entities.timelineEvents")}>
              {events.map((item) => (
                <Link
                  key={item.id}
                  href={`/novels/${novelId}/timeline`}
                  className="block rounded-xl px-2 py-1.5 text-sm text-stone-700 transition hover:bg-white hover:text-stone-950">
                  {item.title || t("timeline.title")}
                </Link>
              ))}
            </RelatedGroup>
          ) : null}
          {adaptations.length > 0 ? (
            <RelatedGroup title={t("entities.adaptations")}>
              {adaptations.map((item) => (
                <Link
                  key={item.id}
                  href={`/novels/${novelId}/adaptations#adaptation-${item.id}`}
                  className="block rounded-xl px-2 py-1.5 text-sm text-stone-700 transition hover:bg-white hover:text-stone-950">
                  {item.title}
                </Link>
              ))}
          ...
          {nextCursor ? (
            <button
              ...
              disabled={loading}>
              {loading ? t("entities.loadingRelatedRecords") : t("common.next")}
            </button>
          ) : null}
        </div>
      )}
